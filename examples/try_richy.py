...
from toolcraft import logger, richy
...
def try_status():
    with richy.Status(tc_log=_LOGGER, title="Test Status") as _s:
...
def try_status_panel():
    _sp = richy.ProgressStatusPanel(title="Test Status Panel", tc_log=_LOGGER)
    with _sp:
        _p = _sp.progress
        _s = _sp.status
        _time = 0.2
    ...
    _sp = richy.ProgressStatusPanel(
        title="Test Status Panel with overall progress",
        tc_log=_LOGGER,
        overall_progress_iterable=[
    ...
    with richy.FitProgressStatusPanel(
        ...
        for _epoch in _status_panel:
            ...
            _train_task = _train_progress.add_task(
                task_name=f"ep {_epoch:03d}",
                total=_train_samples,
                acc=float("nan"),
                loss=float("inf"),
            )
