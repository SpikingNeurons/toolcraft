...
def try_progress():
    ...
    with richy.Progress.for_download_and_hashcheck(
            title="Try Progress Download HashCheck",
            tc_log=_LOGGER,
            console=None,
    ) as _p:
...
def try_status_panel():
    # ------------------------------------------------------------------- 01
    with richy.StatusPanel(tc_log=_LOGGER, title="Test Only Status") as _s:
        _s.update(spinner_speed=1.0,
                  spinner=richy.SpinnerType.dots,
                  status="start")
        time.sleep(1)
        _s.update(spinner_speed=1.0,
                  spinner=richy.SpinnerType.arrow,
                  status="next")
        time.sleep(1)
        _s.update(spinner_speed=1.0,
                  spinner=richy.SpinnerType.balloon,
                  status="done")
        time.sleep(1)
        _s.update(spinner_speed=1.0,
                  spinner=richy.SpinnerType.star,
                  status="close")
        time.sleep(1)
    ...
    for _stage in _sp:
        _sp.update(spinner_speed=1.0,
                   spinner=_stages_meta[_stage],
                   status=_stage)
    ...
    # ------------------------------------------------------------------- 04
    _sp = richy.StatusPanel(
        title="Test Status Panel with generic and stages progress bar",
        tc_log=_LOGGER,
        stages=["start", "next ", "done ", "close"],
    )
    _time = 0.1
    for _i, _stage in enumerate(_sp):
        time.sleep(_time)
        for _ in _sp.track(sequence=[1, 2, 3, 4],
                           task_name=f"task_0 {_i}",
                           prefix_current_stage=True):
            time.sleep(_time)
        _task = _sp.add_task(task_name=f"task_1 {_i}",
                             prefix_current_stage=True,
                             total=4)
        for _ in [0, 1, 2, 3]:
...
def try_fit_progress_status_panel():
    ...
    _train_samples = 5
    _validate_samples = 5
    _fit_panel = richy.StatusPanel(tc_log=_LOGGER,
                                   sub_title=["aaa", "bbb", "ccc"])

    with _fit_panel:

        (
            _train_task_fetcher,
            _validate_task_fetcher,
...
def try_hashable_status_panel():
    ...
    with _rp, _hashable(richy_panel=_rp):
        ...
        for _ in _hashable.richy_panel.track([1, 2, 3, 4],
                                             task_name="task via track()",
                                             msg="fff"):
            time.sleep(0.2)

        # adding task via `add_task()`
        _hashable.richy_panel.update(status="adding task via `add_task()` ...")
        _task = _hashable.richy_panel.add_task(task_name="task via add_task()",
                                               total=5,
                                               msg="rrr")
        for _ in range(5):
        ...
        time.sleep(1)
        _hashable.richy_panel["on the fly 1"] = richy.r_markdown.Markdown(
            "# on the fly \n+ one")
        _hashable.richy_panel["on the fly 2"] = richy.r_markdown.Markdown(
            "# on the fly \n+ two")
        ...
        time.sleep(1)
        _hashable.richy_panel.set_final_message(
            "+ i am done \n+ see you again")
