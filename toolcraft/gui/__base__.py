...
@dataclasses.dataclass
@m.RuleChecker(things_not_to_be_cached=["is_tagged"])
...
USER_DATA = t.Dict[str, t.Union[int, float, str, slice, tuple, list, dict,
                                None, m.FrozenEnum, m.HashableClass, Widget,
                                pathlib.Path, ], ]


@dataclasses.dataclass
@m.RuleChecker(things_not_to_be_cached=["index_in_parent_children"])
...
class Form(MovableWidget, abc.ABC):
    ...
    def build(self) -> t.Union[int, str]:
        ...
        _ch = CollapsingHeader(label=self.title,
