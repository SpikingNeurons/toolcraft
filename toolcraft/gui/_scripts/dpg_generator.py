...
class DpgDef:
    ...
    def _call_prefix(self) -> str:
        ...
        _call_prefix = (
            ...
            else f"internal_dpg.{self.fn.__name__}"
        )
        ...
        for _param in self.fn_signature.parameters.values():
            ...
            if _param_value != inspect._empty:
                if (
                    ...
                    _param_value = (
                        f"dataclasses.field(" f"default_factory=lambda: {_param_value})"
                    )
    ...
    def _code(self) -> t.List[str]:
        ...
        _lines += [
            ...
            *[f"\t\t\t{_pd.dpg_value}," for _pd in self.param_defs if _pd.is_mandatory],
...
class EnumDef:
    ...
    def __init__(self, dpg_enum_prefix: str, dpg_enum_values: t.List[str]):
        ...
        self.enum_name = "En" + \
            self.dpg_enum_prefix.replace("mv", "").replace("_", "")
        self.code = self._code

    def __eq__(self, other):
        return (
            self.dpg_enum_prefix == other.dpg_enum_prefix
...
class CodeMaker:
    ...
    def header(self) -> str:
        ...
        _script_file = _script_file.relative_to(
            _script_file.parent.parent.parent.parent
        ).as_posix()
        _header = f'''"""
********************************************************************************
...
'''
    ...
    def make_widget_py(self):
        ...
        for _widget_def in self.all_dpg_defs:
            if (
                _widget_def.is_plot_related
                or _widget_def.is_table_related
...
def try_dpg_children(_dpg_def: DpgDef, _all_dpg_defs: t.List[DpgDef]):
    ...
    with dpg.window(
        label="Dear PyGui Demo", width=800, height=800, pos=(100, 100), tag="__demo_id"
    ) as w1:
