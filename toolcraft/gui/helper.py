...
from . import __base__, callback, widget


def simple_split_window(
        dash_guid: str,
        title: str) -> t.Tuple[__base__.Dashboard, widget.Group, widget.Group]:
    ...
    _table = widget.Table(
        header_row=False,
        resizable=True,
        policy=widget.TableSizingPolicy.StretchSame,
    ...
    _dash.add_child(guid="t", widget=_table)

    return _dash, _button_cell, _display_cell


def add_widgets_in_line(guid: str, receiver: __base__.Widget,
    ...
    for i, w in enumerate(widgets[1:]):
        receiver.add_child(guid=f"{guid}_{i}_il", widget=widget.InSameLine())
        receiver.add_child(
            guid=f"{guid}_{i+1}",
            widget=w,
        )
...
def tab_bar_from_widget_dict(widget_dict: t.Dict, parent: __base__.Widget):
    ...
    for _k in widget_dict.keys():
        ...
        if isinstance(_v, dict):
            ...
            for _i, __v in enumerate(_v):
                _tab.add_child(guid=f"{_i}", widget=__v)
        else:
...
def tab_bar_from_hashable_callables(
    ...
    for k, v in callable_names.items():
        ...
        _tab_bar.add_child(guid=k, widget=_tab)
        getattr(hashable, v)(binder=__base__.Binder(guid=k, parent=_tab))
...
def button_bar_from_hashable_callables(
    ...
    # add close button
    if close_button:
        _buttons.append(callback.CloseWidgetCallback.get_button_widget())
