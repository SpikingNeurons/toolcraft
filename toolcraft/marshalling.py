...
from . import logger, richy, settings, util
...
class Tracker:
        ...
        **kwargs,
        ...
        if self.is_called:
            ...
            _on_call_kwargs = {
                ...
                "status_panel_progress_task_name":
                status_panel_progress_task_name,
            }
    ...
    def __iter__(self) -> t.Iterable:
        ...
        if _status_panel is None:
            ...
            if self.in_with_context:
                _status_panel.status.update("-- " +
                                            _status_panel_progress_task_name)
                for _ in _status_panel.progress.track(
                        self.on_iter(),
                        total=self.iterable_length,
                        task_name=_status_panel_progress_task_name,
                ):
                    yield _
            else:
                with self:
...
class HashableClass(YamlRepr, abc.ABC):
    ...
    def __post_init__(self):
        ...
        if settings.DO_RULE_CHECK:
            ...
            _modules = [
                _.decorated_class
                for _ in _RULE_CHECKERS_TO_BE_CHECKED.values()
            ]
            if bool(_rc_keys):
                _LOGGER.info(msg=f"Detected {len(_rc_keys)} new classes so "
