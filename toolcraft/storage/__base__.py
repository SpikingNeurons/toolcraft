...
from .. import richy, settings, util
...
class StorageHashable(m.HashableClass, abc.ABC):
    ...
    def uses_parent_folder(self) -> bool:
        ...
        if not _found_file_system:
            ...
            if _found_file_system:
                if not isinstance(getattr(self.__class__, "file_system"),
                                  property):
        ...
        if not _found_parent_folder:
            ...
            if _found_parent_folder:
                if not isinstance(getattr(self.__class__, "parent_folder"),
                                  property):
                    raise e.code.CodingError(msgs=[
        ...
        if not (_found_parent_folder ^ _found_file_system):
            raise e.code.CodingError(msgs=[
                ...
                dict(
                    _found_file_system=_found_file_system,
    ...
    def is_created(self) -> bool:
        ...
        if _info_there ^ _config_there:
            raise e.code.CodingError(msgs=[
                ...
                f"be present ...",
                dict(_info_there=_info_there, _config_there=_config_there),
            ])
    ...
    def init_validate(self):
        ...
        # if not _uses_parent_folder then test if valid file_system
            ...
            e.validation.ShouldBeOneOf(
                value=self.file_system,
                values=_fs.available_file_systems(),
                msgs=["Expecting file_system to be valid ..."],
            ).raise_if_failed()
        ...
        if self.path.exists():
            if not self.path.isdir():
                raise e.validation.NotAllowed(
                    msgs=[f"We expect {self.path} to be a dir"])
    ...
    def from_dict(cls, yaml_state: t.Dict[str,
                                          "m.SUPPORTED_HASHABLE_OBJECTS_TYPE"],
                  **kwargs) -> "StorageHashable":
    ...
    def create_post_runner(
            self, *, hooked_method_return_value: t.Union[_fs.Path,
                                                         t.List[_fs.Path]]):
