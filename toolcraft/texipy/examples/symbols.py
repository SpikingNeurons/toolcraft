...
paperkeywords = Command(latex="multi-trunk "
                        "\\and low accuracy "
                        "\\and data-augmentation "
                        "\\and improve accuracy "
                        "\\and order vs. chaos ")

tracewithid = Command(latex="\\(t_{#1}^{(#2)}\\)",
                      num_args=2,
                      default_value="*")
...
mtovc = Acronym(short_name="MTOvC",
                full_name="\\textit{multi-trunk order vs. chaos}")
gllabel = Glossary(
    name="$(label)$",
...
glacc = Glossary(name="$Acc$", description="Accuracy of the classifier.")
glbacc = Glossary(
    name="$Acc_{base}$",
...
gldssim = Glossary(name="\\texttt{simulated}",
                   description="Simulated dataset.")

gldssimnoisy = Glossary(name="\\texttt{simulated-noisy}",
                        description="Simulated dataset with noise.")

gldsascadvonefk = Glossary(name="\\texttt{ascad-v1-fk}",
                           description="ASCAD v1 dataset for fixed key.")

gldsascadvonevk = Glossary(name="\\texttt{ascad-v1-vk}",
