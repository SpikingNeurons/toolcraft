...
from toolcraft.texipy import Color, List, Section, SubSection, beamer, tikz
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------ add all sections
    _section_1 = Section(label="sec:list", name="Lists ...")
    _section_2 = Section(label="sec:tikztable", name="TikZ and Table ...")
    _section_ref = Section(label="sec:ref", name="References ...")
    _ppt.add_item(_section_1).add_item(_section_2).add_item(_section_ref)
    ...
    _frame_list1 = beamer.Frame(
        ...
        type="itemize",
        items=["Apple", "Mango", ("$\\blacksquare$", "Papaya"), "Orange"],
    ))
    ...
    _section_2_1.add_item(
        beamer.Frame(title="TikZ figure", label="tikz").add_item(
            item=try_tikz.make_complicated_figure(scale=(0.8, 0.61))))
    _section_2_2.add_item(
        beamer.Frame(title="Table", label="table").add_item(
