...
def make_label_nodes(_tikz: tikz.TikZ) -> t.Dict[str, t.Dict[int, tikz.Node]]:
    ...
    _ret["hw"][1] = tikz.Node(id="hw01",
                              text_to_display=symbols.labelwithlk("hw", "1"))
...
def make_shaded_clusters(_tikz: tikz.TikZ,
                         _point_nodes: t.Dict[int, t.List[tikz.Node]]):
    _path = tikz.Path(style=tikz.Style(
        fill=tikz.FillOptions(color=Color.gray(intensity=30), ),
...
def make_point_nodes(_tikz: tikz.TikZ,
                     apply_style: bool) -> t.Dict[int, t.List[tikz.Node]]:
    _path = tikz.Path()
    ...
    for _label in range(4):
        ...
        if apply_style:
            _point_style = tikz.Style(
                draw=tikz.DrawOptions(color=_color,
                                      thickness=tikz.Thickness.thin),
                fill=tikz.FillOptions(color=_color(intensity=30)),
                shape=tikz.Circle(),
            )
        else:
            _point_style = None
        ...
        for _p in range(4):
            ...
            _node_text = symbols.tracewithid(str(_iths[_label * 4 + _p]),
                                             str(_label))
            _node = tikz.Node(
                style=_point_style,
...
def make_edges(
    ...
    _edge_style_easy = tikz.Style(draw=tikz.DrawOptions(
        color=Color.green,
        thickness=tikz.Thickness.thin,
        arrow_def=tikz.ArrowDef(end_tips=tikz.ArrowTip.latex, ),
    ))
    _edge_style_difficult = tikz.Style(draw=tikz.DrawOptions(
        color=Color.red,
    ...
    _ambig_legend_node = tikz.Node(
        id="ambignode",
        anchor=tikz.Anchor.below(offset=_HEIGHT * 0.01),
        text_to_display=FontSize.scriptsize("Ambiguous labels"),
        style=tikz.Style(shape=tikz.Rectangle(minimum_height=_HEIGHT * 0.05)),
    )
    ...
    _ambig_legend_path.move_to(_ambig_legend_node.south_west).line_to(
        _ambig_legend_node.south_east)
    _tikz.add_path(_ambig_legend_path)

    _nonambig_legend_path = tikz.Path(style=_edge_style_easy)
    _nonambig_legend_path.move_to(_nonambig_legend_node.south_west).line_to(
        _nonambig_legend_node.south_east)
...
def make_complicated_figure(scale: t.Tuple[float, float] = None):
    ...
    _tikz = tikz.TikZ(
        caption="Learnable Labels Vs. Ambiguous Labels",
        label="fig:ambig",
