...
def parse_attr(f, bpc, chunk):
    ...
    stype, sname, sparser = ATTR_INFO.get(
        type, ("unk_%d" % type, str(type), parseRaw))
    if sparser is None:
        sparser = parseRaw
    sname = sname.strip()
    ...
    if nonres:
        ...
        def attrdata():
            return sparser(read_runlist(f, bpc, runlist)[:size_actual])
...
def main(argv):
    ...
    mft_clust, mftmirr_clust, clust_per_mft = struct.unpack(
        "<QQB", readat(f, 0x30, 17))
